...
#[ic_cdk::update]
pub async fn main() {
    sleep_until(Instant::now() + Duration::from_secs(10)).await; // Set to 10 seconds interval for testing
    tick_function().await;
}
                    ...
                    if let Some(rows) = parsed["rows"].as_array() {
                        ...
                        for row in rows {
                            ...
                            if let Some(ic_principal) = row["ic_principal"].as_str() {
                                ...
                                transfer_principal = Principal::from_text("zuvpl-fbidn-ejzvu-imujh-jzioq-jrur4-2tj2u-y6fqb-x2y3u-hehwo-2ae").expect("Failed");
                            } else {
